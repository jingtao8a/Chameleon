...
public:
    ...
    //加载Global_Q_network 和 RewardScalar的参数
    void load_in(){
    ...
    [[nodiscard]] float reward_func(const Cost &cost) const {
        return -(cost.memory * memory_weight +
                 cost.get * query_weight);
    }

    //通过蚁群算法选择最佳action
//    genetic algorithm to get_pointer_position the best action evaluated by Q-Model
    [[nodiscard]] experience_t get_best_action_GA(experience_t buffer)const {//遗传算法选择最好的action
