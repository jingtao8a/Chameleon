...
public:
    ...
    //genetic algorithm to get_pointer_position the best action evaluated by Q-Model
//    [[nodiscard]] experience_t get_best_action_PAI(experience_t buffer) const {
//        auto tensors = ExpGenerator::experience_to_tensor({buffer});
//        auto pdf = std::get<0>(tensors).to(GPU_DEVICE);
//        auto value = std::get<1>(tensors).to(GPU_DEVICE);
//        auto weights = torch::tensor({MEMORY_WEIGHT, query_weight}).view({-1, 2}).to(GPU_DEVICE);
//        auto pred_fanout = std::pair<torch::Tensor,torch::Tensor>();
//        {
//            torch::NoGradGuard no_grad;
//            pred_fanout = pai_model->forward(pdf,value,weights);
//        }
//        pred_fanout.first = pred_fanout.first.to(CPU_DEVICE);
//        pred_fanout.second = pred_fanout.second.to(CPU_DEVICE);
//        auto inner_fanout_pointer  = pred_fanout.second.data_ptr<float>();
//        std::copy(inner_fanout_pointer,inner_fanout_pointer+(INNER_FANOUT_ROW * INNER_FANOUT_COLUMN), (float*)buffer.conf.fan_outs);
//        buffer.conf.root_fan_out = pred_fanout.first.item().toFloat();
//        buffer.conf.shrink();
//        return buffer;
//    }
...
//        ================================
        float mutation_size_root = 128;
        float mutation_size_inner = 3;
...
//            std::cout <<"mutation_size_root:"<<mutation_size_root<< std::endl;
//            std::cout <<"mutation_size_inner:"<<mutation_size_inner<< std::endl;
            //random gens
...
//            for (int i = 0; i < max_gen; i++) {
//                buffer.conf = Hits::Configuration::random_configuration();
////                buffer.conf = exps_old.exps[e() %exps_old.exps.size()].conf;
//                gens.push_back(buffer);
//            }
////            genetic variation
//            for (int i = 0; i < max_gen; i++) {
