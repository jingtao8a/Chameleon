...
//        auto dataset = dataset_source::get_dataset<std::pair<KEY_TYPE, VALUE_TYPE>>(train_dataset_path + "books_uint64_without_duplicates.bin");
...
//                exp.memory = float(Hits::node_memory_count - last_memory)/ sizeof(Hits::DataNode<KEY_TYPE,VALUE_TYPE>::Slot)/ float(std::max(1,exp.dataset_size));
                exp.reward = -GET_WEIGHT * inner_cost_weight - MEMORY_WEIGHT * float(Hits::node_memory_count - last_memory) / sizeof(Hits::DataNode<KEY_TYPE,VALUE_TYPE>::slot_type) / float(std::max(1, exp.dataset_size));
...
//                if(double(Hits::leaf_cost)  / double(std::max(exp.dataset_size,1)) > 10){
//                    std::cout <<"leaf cost:"<<double(Hits::leaf_cost)  / double(std::max(exp.dataset_size,1))<< std::endl;
//                    std::cout <<"leaf size:"<<data_node->size<<"  capacity:"<<data_node->capacity<< std::endl;
//                    sleep(1);
//                }
                exp.reward = -GET_WEIGHT * inner_cost_weight - float(double(Hits::leaf_cost) / double(std::max(exp.dataset_size, 1)))
...
//        std::cout <<  tc.get_timer_nanoSec() / double(dataset.size())<< std::endl;
        ...
        delete index;
        {
            for(auto i:id_to_node){//a batch s,a,r,s'
                ...
                if(int(father_id_to_ids[node_id].size()) != 0){//this node hash sons
