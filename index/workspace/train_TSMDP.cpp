...
public:
    //state
    float pdf[SMALL_PDF_SIZE]{};//存储数据集的概率密度函数
    int dataset_size = 0;//数据集大小
    //action （fanout）
    int action = 0;//采取的动作
    //reward
    float reward = 0;//获得的奖励
};
...
std::tuple<torch::Tensor,torch::Tensor,torch::Tensor,torch::Tensor> exp_small_to_tensor(const std::vector<ExpSmall>& exps){
    ...
    for (auto &i:exps) {// 遍历 exps 向量中的每个 ExpSmall 实例
        // 将 ExpSmall 实例的 pdf 数组复制到第一个张量中
        std::copy(i.pdf, i.pdf + SMALL_PDF_SIZE, pdf_ptr);
        pdf_ptr += SMALL_PDF_SIZE;
        ///////////////////////////
        // 将 dataset_size 的值复制到第二个张量的第一个元素
        value_ptr[0] = float(i.dataset_size);
        value_ptr += VALUE_SIZE;
        ////////////////////////////
        // 将 action 的值复制到第三个张量的第一个元素
        action_ptr[0] = float(i.action);
        action_ptr += 1;
        // 将 reward 的值复制到第四个张量的第一个元素
        reward_ptr[0] = i.reward;
        reward_ptr += 1;
    }
    return result;// 返回包含四个张量的元组
}
...
std::ofstream log_file("logfile.txt");
 int main(){
    ...
    //初始化q_network和q_target_network
     ...
     float random_rate = 1;//用于选择action时
     float discount_rate = 0.9;//折扣率

     //遍历所有的数据dataset排好序放入datasets中
     std::vector<std::string> dataset_names = scanFiles(data_father_path);
     std::vector<std::vector<std::pair<KEY_TYPE,VALUE_TYPE >>> datasets;
     for(auto i:dataset_names){
         auto dataset = dataset_source::get_dataset<std::pair<KEY_TYPE, VALUE_TYPE>>(data_father_path + i);
     ...
     for(int tree_id = 0;;++tree_id){//进入训练大循环
         std::cout <<"tree id:"<<tree_id<<"  random rate:"<<random_rate<< std::endl;
         //将exp_tensors中的tuple打乱顺序 start
        ...
        //以下为 构建树状马尔科夫决策过程(TSMDP) 所需要存储的一些信息
        ...
        //随机选取dataset，并截断为random_length长度
         auto dataset = datasets[tree_id % datasets.size()];//根据tree_id选择一个dataset
         ...
         std::cout <<"random_length:"<<random_length<< std::endl;//生成随机数random_length
         auto random_start = (int) (e() % (dataset.size() - random_length));
         dataset.erase(dataset.begin(), dataset.begin() + random_start);
         dataset.erase(dataset.begin() + random_length, dataset.end());//将dataset的后random_length的key保存，其它的erase
         ...
         Hits::Index<KEY_TYPE,VALUE_TYPE> *index;//创建Hit
         {
         ...
         int son_id = -1;
         while(!tasks.empty()){//使用q_target_network构建hits
             ...
             auto pdf = get_pdf<KEY_TYPE, VALUE_TYPE>(
                     ...
                     task.lower, task.upper, SMALL_PDF_SIZE);//根据task生成pdf
             std::copy(pdf.begin(),pdf.end(),exp.pdf);
             exp.dataset_size = task.stop - task.start;//根据task生成value
             ...
             if(exp.action > 0 && exp.dataset_size > DATA_NODE_SIZE){
                 ...
                 auto inner_node = Hits::InnerNode<KEY_TYPE,VALUE_TYPE>::new_segment(1 << int(exp.action),task.lower,task.upper);
 ...
 //                exp.memory = float(Hits::node_memory_count - last_memory)/ sizeof(Hits::DataNode<KEY_TYPE,VALUE_TYPE>::Slot)/ float(std::max(1,exp.dataset_size));
                 ...
                 auto last_memory = Hits::node_memory_count;
                 auto data_node = Hits::DataNode<KEY_TYPE,VALUE_TYPE>::new_segment(std::max(DATA_NODE_SIZE,int(float(std::max(1,exp.dataset_size)) / default_density)),task.lower,task.upper);
         ...
         for(auto const &i:father_id_to_ids){
             ...
             if(i.first != -1 && ((Hits::InnerNode<KEY_TYPE,VALUE_TYPE>*)id_to_node[i.first])->capacity != int(i.second.size())){
         ...
         delete index;
         {
             for(auto i:id_to_node){//a batch s,a,r,s'
                 ...
                 if(int(father_id_to_ids[node_id].size()) != 0){//this node has sons
