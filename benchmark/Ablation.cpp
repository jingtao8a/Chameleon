//
// Created by redamancyguy on 24-2-9.
//
#include <iostream>
#include <random>
#include<functional>
...
int main() {
    ...
    if (IsFileExist((father_path + "read_only/read_only_result_ab.txt").c_str())) {
        std::time_t t = std::time(nullptr);
        std::stringstream string_s;
        string_s << std::put_time(std::localtime(&t), "%Y-%m-%d %H.%M.%S");
        rename((father_path + "read_only/read_only_result_ab.txt").c_str(),
               (father_path + "read_only/read_only_result_ab" + string_s.str() + ".txt").c_str());
    }
    std::ofstream result(father_path + "read_only/read_only_result_ab.txt", std::ios::out | std::ios::binary);
    std::string dis_type;
