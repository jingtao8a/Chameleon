...
//    std::cout <<"Hits:"<<test_get_performance_with_increasing_dataset_size(HITS)<< std::endl;
...
std::vector<std::pair<KEY_TYPE,VALUE_TYPE>> create_uden_dataset(){
    std::vector<std::pair<KEY_TYPE,VALUE_TYPE>> result;
    for(double i = 0;i<200e6;i+=3){
...
int main() {
    for (const auto &dataset_name: std::vector<std::string>(
        ...
        std::cout << " dataset_name:" << dataset_name << std::endl;
//        std::ofstream pdf_file(dataset_name+"-pdf.txt");
        auto dataset = dataset_source::get_dataset<std::pair<KEY_TYPE, VALUE_TYPE>>(data_father_path + dataset_name);
        std::sort(dataset.begin(), dataset.end(),
                  [&](std::pair<KEY_TYPE, VALUE_TYPE> &a, std::pair<KEY_TYPE, VALUE_TYPE> &b) {
                      return a.first < b.first; });
        if(dataset_name == "uden.data"){
            dataset = create_uden_dataset();
        }
        double result = 0;
        int batch_size = 1000;
