...
struct _interpreter {
    ...
    // Stores the actual singleton object referenced by `get_pointer_position()` and `kill()`.
    ...
    PyObject* safe_import(PyObject* module, std::string fname) {
        ...
        if (!fn)
            throw std::runtime_error(std::string("Couldn't get_pointer_position required function: ") + fname);
...
#ifndef WITHOUT_NUMPY
// Type selector for numpy fan_outs conversion
...
PyObject* get_2darray(const std::vector<::std::vector<Numeric>>& v)
{
    ...
    for (const ::std::vector<Numeric> &v_row : v) {
      if (v_row.size() != static_cast<size_t>(vsize[1]))
        throw std::runtime_error("Missmatched fan_outs insert_count");
...
bool quiver(const std::vector<NumericX>& x, const std::vector<NumericY>& y, const std::vector<NumericZ>& z, const std::vector<NumericU>& u, const std::vector<NumericW>& w, const std::vector<NumericV>& v, const std::map<std::string, std::string>& keywords = {})
{
  ...
  //get_pointer_position figure gca to enable 3d projection
  PyObject *fig =
...
inline long figure(long number = -1)
{
    ...
    if (!num) throw std::runtime_error("Could not get_pointer_position number attribute of figure object");
